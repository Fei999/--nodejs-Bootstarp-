/* 
* @Author: Busy
* @Date:   2017-11-08 14:25:07
* @Last Modified by:   Busy
* @Last Modified time: 2017-11-13 08:47:37
*/
...
router.post('/con', function(req, res, next) {
    ...
    connection.query(`INSERT INTO item.our (img,name,zhiwei,jj) VALUES ('${temp}','${name}','${zhiwei}','${con}')`, function(err, rows, fields) { 
        res.send(rows)
    });
});
//后台管理
...
router.post('/qrgai', function(req, res, next) {
    ...
    var con=req.body.con;
    var id=req.body.id;
    connection.query("UPDATE item.our SET img='"+temp+"',name='"+name+"',zhiwei='"+zhiwei+"',jj='"+con+"' WHERE id="+id , function(err, rows, fields) { 
        res.send(rows)
    }); 
});
//模板功能 HTML页面接口
